...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <aside className={`relative flex flex-col h-full max-h-screen bg-[#0B1120] border-r border-slate-800/80 transition-all duration-300 z-30 flex-shrink-0 overflow-hidden ${isCollapsed ? 'w-20' : 'w-72'}`}>
      {/* Collapse Toggle */}
      <button
      ...
      {/* Header */}
      <div className="p-4 flex items-center justify-between border-b border-slate-800/60 flex-shrink-0">
      ...
      <div className="p-3 flex-shrink-0">
        <button
          onClick={onNewChat}
          className={`w-full py-2.5 rounded-xl bg-gradient-to-r from-[#5B5FFF] to-[#7B61FF] text-white font-semibold text-sm hover:shadow-glow transition-all flex items-center justify-center gap-2 active:scale-95 ${isCollapsed ? 'px-0' : 'px-4'}`}
          title="New Chat"
        >
      ...
      {!isCollapsed ? (
        <div className="mx-3 my-2 p-3 rounded-xl glass-panel border border-slate-800 flex-shrink-0">
      ...
      {/* Footer Controls */}
      <div className="p-3 border-t border-slate-800/80 space-y-1 flex-shrink-0">
        <button
